...
public class BoardDAO {
	...
	//무한 스크롤
	private final String SQL_SELECTALL_MAIN = "SELECT * FROM (SELECT B.BNUM, COUNT(*) ACNT FROM BOARD B LEFT JOIN APPLICANT A ON B.BNUM=A.BNUM GROUP BY B.BNUM) C, "
			+ "(SELECT BNUM, B.MNUM, BTITLE, BRATE, BCNT, BDATE, BLOCAL, BACTION, MID  FROM BOARD B INNER JOIN MEMBER M ON M.MNUM=B.MNUM WHERE BACTION=0 ORDER BY BDATE ASC LIMIT 1000000) A "
			+ "WHERE C.BNUM=A.BNUM "
			+ "UNION ALL "
			+ "SELECT * FROM (SELECT B.BNUM, COUNT(*) ACNT FROM BOARD B LEFT JOIN APPLICANT A ON B.BNUM=A.BNUM GROUP BY B.BNUM) C, "
			+ "(SELECT BNUM, B.MNUM, BTITLE, BRATE, BCNT, BDATE, BLOCAL, BACTION, MID FROM BOARD B INNER JOIN MEMBER M ON M.MNUM=B.MNUM WHERE BACTION=1 ORDER BY BDATE ASC LIMIT 1000000) B "
			+ "WHERE C.BNUM=B.BNUM AND B.BNUM LIMIT ?,?";
	// 등록순
	private final String SQL_SELECTALL_REGISTER = "SELECT * FROM (SELECT B.BNUM, COUNT(*) ACNT FROM BOARD B LEFT JOIN APPLICANT A ON B.BNUM=A.BNUM GROUP BY B.BNUM) C, "
			+ "(SELECT BNUM, B.MNUM, BTITLE, BRATE, BCNT, BDATE, BLOCAL, BACTION, MID  FROM BOARD B INNER JOIN MEMBER M ON M.MNUM=B.MNUM WHERE BACTION=0 ORDER BY BWDATE ASC LIMIT 1000000) A "
			+ "WHERE C.BNUM=A.BNUM "
			+ "UNION ALL "
			+ "SELECT * FROM (SELECT B.BNUM, COUNT(*) ACNT FROM BOARD B LEFT JOIN APPLICANT A ON B.BNUM=A.BNUM GROUP BY B.BNUM) C, "
			+ "(SELECT BNUM, B.MNUM, BTITLE, BRATE, BCNT, BDATE, BLOCAL, BACTION, MID FROM BOARD B INNER JOIN MEMBER M ON M.MNUM=B.MNUM WHERE BACTION=1 ORDER BY BWDATE ASC LIMIT 1000000) B "
			+ "WHERE C.BNUM=B.BNUM";
	// 시합 날짜순
	...
	private final String SQL_SELECTALL_AREA = "SELECT * FROM (SELECT B.BNUM, COUNT(*) ACNT FROM BOARD B LEFT JOIN APPLICANT A ON B.BNUM=A.BNUM GROUP BY B.BNUM) C, "
			+ "(SELECT BNUM, B.MNUM, BTITLE, BRATE, BCNT, BDATE, BLOCAL, BACTION, MID FROM BOARD B INNER JOIN MEMBER M ON M.MNUM=B.MNUM WHERE BLOCAL=? AND BACTION = 0 ORDER BY BDATE ASC LIMIT 1000000) A "
			+ "WHERE C.BNUM=A.BNUM "
			+ "UNION ALL "
			+ "SELECT * FROM (SELECT B.BNUM, COUNT(*) ACNT FROM BOARD B LEFT JOIN APPLICANT A ON B.BNUM=A.BNUM GROUP BY B.BNUM) C, "
			+ "(SELECT BNUM, B.MNUM, BTITLE, BRATE, BCNT, BDATE, BLOCAL, BACTION, MID FROM BOARD B INNER JOIN MEMBER M ON M.MNUM=B.MNUM WHERE BLOCAL=? AND BACTION = 1 ORDER BY BDATE ASC LIMIT 1000000) B "
			+ "WHERE C.BNUM=B.BNUM";
	// 날짜 선택
	...
	// 헤더 부분 검색(제목만 가능)
	private final String SQL_SELECTALL_SEARCH = "SELECT * FROM (SELECT B.BNUM, COUNT(*) ACNT FROM BOARD B LEFT JOIN APPLICANT A ON B.BNUM=A.BNUM GROUP BY B.BNUM) C, "
			+ "(SELECT BNUM, B.MNUM, BTITLE, BRATE, BCNT, BDATE, BLOCAL, BACTION, MID FROM BOARD B INNER JOIN MEMBER M ON M.MNUM=B.MNUM WHERE BACTION != 2 ORDER BY BDATE ASC LIMIT 1000000)A "
			+ "WHERE C.BNUM=A.BNUM AND BTITLE LIKE CONCAT('%',?,'%')";
	...
	public List<BoardVO> selectAllMain(BoardVO bvo) {
		...
		try {
			if (bvo.getSortBy() != 0) { // 정렬
				...
				} else if (bvo.getSearchContent() != null) { // 헤더 검색
