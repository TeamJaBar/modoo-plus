...
TRUNCATE TABLE MORDER CASCADE CONSTRAINTS;

UPDATE MEMBER SET MPW='abc123123123' WHERE MNUM=-1;
UPDATE MEMBER SET MNUM=-1 WHERE MNUM=0;
UPDATE MEMBER SET MEMAIL='polodltjdgns@naver.com' WHERE MNUM=12;

SELECT * FROM (SELECT CASE D.MNUM  WHEN 12 THEN 1 ELSE 0 END DIB, P.PNUM, P.CATENUM, P.PNAME, P.FIXPRICE, P.SELPRICE, P.RDATE, P.REPERSON, P.REAGE, P.BRAND, P.PIMG, P.PRODUCTCNT, CASE WHEN O.CNT IS NULL THEN 0 ELSE O.CNT END OCNT FROM PRODUCT P LEFT OUTER JOIN DIB D ON P.PNUM = D.PNUM FULL JOIN (SELECT PNUM, SUM(CNT) CNT FROM ORDERDETAIL GROUP BY PNUM) O ON P.PNUM=O.PNUM ORDER BY OCNT DESC) WHERE ROWNUM<3;
...
UPDATE MEMBER SET MPOINT = 3500 WHERE MNUM = 12;
